...
//after:  0  8  4  12 2  10 6  14 1  9  5  13 3  11 7  15
//before
...
*/
void    conv(float *s, int ns, float *f, int n, float *g) {
  int   i,j,k,m;
  float *pt;
  m = n+ns;
//fprintf(stderr,"s and g in conv: %d %d\n",s,g);
...
*/
float	*crscrl(int npt,int nft,int m,float *rec,float *syn,float *ss1,float *ss2,float *ss3) {
  int	i,nft2;

  nft2=nft;nft*=2;
  for(i=0;i<nft;i++) {ss1[i]=0.;ss2[i]=0.;}
...
//  for(i=npt;i<nft;i++) {ss1[i]=0.;ss2[i]=0.;}
  ...
  nft2 -= m/2;
  memcpy(ss3, ss1+nft2, (m+1)*sizeof(float));
  return(ss3);
}
...
//  crs = crscrl(n,data,syn,2*max_shift);     /* tempeprarily hided for the modified 'crscrl' in mcmc */
