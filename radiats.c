...
void	dc_radiat(double stk,double dip,double rak,double rad[3][3]) {
   double sstk,sdip,srak,sstk2,sdip2;
   double cstk,cdip,crak,cstk2,cdip2;
...
******************************************************/
void	sf_radiat(double stk,double dip,double rad[3][3]) {
   double sstk,sdip,cstk,cdip;
   stk*=RAD; dip*=RAD;
   sstk=sin(stk);cstk=cos(stk);
   sdip=sin(dip);cdip=cos(dip);
   rad[0][0]=-sdip;
...
void	mt_radiat(double az, double m[3][3], double rad[3][3]) {
   double saz, caz, saz2, caz2;
   az *= RAD;
